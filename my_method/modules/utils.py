...
def save_training_history_csv_multitask(
    ...
    for epoch in range(num_epochs):
        row = {
            ...
            "train_consistency_loss": train_history.get("consistency_loss", [0.0])[epoch] if epoch < len(train_history.get("consistency_loss", [])) else 0.0,
            ...
            "val_consistency_loss": val_history.get("consistency_loss", [0.0])[epoch] if epoch < len(val_history.get("consistency_loss", [])) else 0.0,
        }
        ...
        if test_history is not None and epoch == num_epochs - 1:
            ...
            row["test_consistency_loss"] = test_history.get("consistency_loss", 0.0)
        else:
            row["test_total_loss"] = ""
